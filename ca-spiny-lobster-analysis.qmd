...
# to start, we'll make individual subsets for each site:


aque_subset <- lobster_temps_august %>% 
  filter(site %in% c("AQUE"))

carp_subset <- lobster_temps_august %>% 
  filter(site %in% c("CARP"))

ivee_subset <- lobster_temps_august %>% 
  filter(site %in% c("IVEE"))

mohk_subset <- lobster_temps_august %>% 
  filter(site %in% c("MOHK"))

napl_subset <- lobster_temps_august %>% 
...
simple_linear_reg <- lm(formula = avg_size ~ avg_temp, data = )
